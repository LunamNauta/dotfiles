...
Singleton {
    ...
    function alterColour(c: color, a: real, layer: int): color{
        ...
        const offset = 0.3 * (1 - transparency.base) * (1 + 1 * 2.5); // TODO: Fix. This line was butchered
    ...
    function layer(c: color, layer: var): color{
        if (!transparency.enabled) return c;
        return layer === 0 ? Qt.alpha(c, transparency.base) : alterColour(c, transparency.layers, layer ?? 1);
    }

    function on(c: color): color{
        if (c.hslLightness < 0.5) return Qt.hsla(c.hslHue, c.hslSaturation, 0.9, 1);
    ...
    function load(data: string): void{
        ...
        for (const [name, color] of Object.entries(scheme.colors)) {
            const prop_name = name.startsWith("term") ? name : `m3${name}`;
            if (colors.hasOwnProperty(prop_name)) colors[prop_name] = `#${color}`;
        }
    }
