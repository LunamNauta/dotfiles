...
Singleton{
    ...
    readonly property var weather_icons: ({
        "113": "clear_day",
    ...
    function getAppCategoryIcon(name: string, fallback: string): string {
        ...
        for (const [key, value] of Object.entries(category_icons)) if (categories.includes(key)) return value;
        return fallback;
    }
    ...
    function getWeatherIcon(code: string): string {
        if (weather_icons.hasOwnProperty(code)) return weather_icons[code];
        return "air";
    }
    ...
    function getMicVolumeIcon(volume: real, is_muted: bool): string{
        if (!is_muted && volume > 0) return "mic";
        return "mic_off";
    }
