...
Singleton{
    ...
    property real storage_perc: storage_total > 0 ? storage_used / storage_total : 0
            ...
            onStreamFinished: {
                ...
                for (const line of text.trim().split("\n")){
                    if (line === "Adapter: PCI adapter") eligible = true;
                        ...
                        if (!match) match = line.match(/^(junction|mem)+:\s+\+([0-9]+\.[0-9]+)(°| )C/); // Fall back to junction/mem if GPU doesn't have edge temp (for AMD GPUs)
                        if (match) {
